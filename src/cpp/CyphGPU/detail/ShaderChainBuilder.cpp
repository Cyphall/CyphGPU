...
cgpu::detail::ShaderChainBuilder::ShaderChainBuilder(std::span<const vk::DescriptorSetAndBindingMappingEXT> mappings):
	m_mappings{mappings}
{
}
...
)
{
	...
	mapping_info.mappingCount = static_cast<uint32_t>(m_mappings.size());
	mapping_info.pMappings = m_mappings.data();
}
